...
function maybeConvertDashToBullet() {
  ...
  // Check conditions before any DOM manipulation so the live range stays valid.
  if (range.startOffset !== 2) return;
  // Chrome converts trailing spaces in contenteditable to NBSP to prevent
  // whitespace collapsing, so accept either regular space or NBSP after the dash.
  if (!/^-[ \u00A0]/.test(node.textContent)) return;

  // Find the top-level block in preview that contains the cursor.
  let block = node.parentElement;
  const isDirectChild = (block === preview);
  ...
  if (!isDirectChild) {
    ...
    // Cursor must be in the very first text node of the block.
    const walker = document.createTreeWalker(block, NodeFilter.SHOW_TEXT);
    if (walker.nextNode() !== node) return;
  }

  // Build the replacement list before touching the DOM.
  const remaining = node.textContent.slice(2);
  const ul = document.createElement('ul');
  const li = document.createElement('li');
  const liText = document.createTextNode(remaining || '\u00A0');
  li.appendChild(liText);
  ul.appendChild(li);

  if (isDirectChild) {
    // Text node sits directly in preview — swap it out in place.
    preview.replaceChild(ul, node);
  } else {
    // Strip the "- " from the block and move its remaining children into li.
    node.textContent = remaining || '\u00A0';
    while (block.firstChild) li.insertBefore(block.firstChild, liText);
    block.parentNode.replaceChild(ul, block);
  }

  // Put the caret at the start of the new li.
  const newRange = document.createRange();
  newRange.setStart(liText, 0);
  newRange.collapse(true);
  sel.removeAllRanges();
  sel.addRange(newRange);
  scheduleSave();
